    ...
    return `
      <div class="tooltip-metric-card">
        <div class="tooltip-metric-content">
          ...
          ${
            sparkline
              ? `<div class="sparkline-container">${sparkline.outerHTML}</div>`
              : ""
          }
        </div>
      </div>
...
async function initializeMap(stateData) {
  ...
  // Create color scale
  const colorScale = d3.scaleThreshold().domain([1001, 2001, 3001]).range([
    "#AADCD9", // Light blue, 1-1,000
    "#76CABB", // Light teal, 1,001-2,000
    "#4BA8C9", // Teal, 2,001-3,000
    "#26347E", // Dark blue, 3,001+
  ]);
