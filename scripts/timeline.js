...
function initializeTimeline(startYear, endYear, yearCallback) {
  ...
  const customThumb = thumbContainer.append('div').attr('class', 'custom-thumb')
  ...
  function play() {
    ...
    // Reset to start if we're at the end year
    ...
    interval = setInterval(() => {
      currentYear = parseInt(timeline.property('value'))
      if (currentYear >= endYear) {
        pause()
        return
      }
